...
When TypeScript strict null checks are enabled, compile errors occur if you don't explicitly handle `undefined` and `null`.
...
When strict null checks are enabled, TypeScript will not compile otherwise.

## Type Safety

### Strict Mode

Follow your project `tsconfig` policy:
- Active development may use relaxed settings (see `typescript-config-guide.md`)
- Production/stabilization should enable strict mode
